<?php
	/*
	*	Policy vote processing
	*/

	// Is policy adopted?
	function is_policy_adopted_function() {
        $gameID = $_POST['gameID']; 
		...
		$thePolicy = Array(
			"ID" => $policyID,
			"name" => get_the_title($policyID),
			"policy_budgetary_cost_per_capita" => get_post_meta($policyID, '_policy_budgetary_cost_per_capita', true),
			"happiness_score_effects" => get_post_meta($policyID, '_happiness_score_effects', true),
		);

        if (!empty($policies_instance)) {
        	// Active policies found, so loop through them and check if the right one exists, if does send it to client
			$isactive = false;
			$policy_instance = Array();
	...
	add_action( 'admin_post_nopriv_is_policy_adopted', 'is_policy_adopted_function' );
	add_action( 'admin_post_is_policy_adopted', 'is_policy_adopted_function' );

	// Put policy adoption / change / remove to vote
	function put_legislation_to_vote_function() {

		$gameCode = $_POST['gameCode'];
		$gameID = $_POST['gameID'];
		$legislationID = $_POST['legislationID'];
		$level = $_POST['level'];
		...
		// Get policy name
		$policy_name = get_the_title($legislationID);

        // Get current level of the policy
        $active_policy_instance = null;
        foreach ($policies_instance as $key => $active_policy) {
        ...
        foreach ($country_instance['parties'] as $key => $party) {
        	...
        	if ($forOrAgainst > 0) {
        		...
        		if ($splitChoice <= 2) {
        			...
        			$partyIsForPolicy = true;
        			$partyline = "for";
        		} else if ($splitChoice > 2 && $splitChoice <= 5) {
        			// Split party is voting empty (30% chance)
        			$empty = $empty + $seats;
        			$partyline = "empty";
        		} else if ($splitChoice > 5 && $splitChoice <= 10) {
        			// Split party is against (50% chance)
        ...
        foreach ($country_instance['parties'] as $key => $party) {
            ...
            // +-60% each way random
            $gallup_change = $gallup_change * random(0.4, 1.2);
        ...
        // Edit the game, so that the policy effects take place
        if ($vote_passed && !$removal) {
            // Adopt policy
            $adoptPolicyResults = adoptPolicy($gameID, $legislationID, $level, $proposed_by, $policies_instance, $country_instance);
		...
		$_SESSION['legislationID'] = $legislationID;
	...
	add_action( 'admin_post_nopriv_put_legislation_to_vote', 'put_legislation_to_vote_function' );
	add_action( 'admin_post_put_legislation_to_vote', 'put_legislation_to_vote_function' );
