...
function getStatsNow($gameID) {
	...
	$statsNow = Array(
		...
		"inflation_add_per_turn" => (double)get_post_meta( $gameID, '_inflation_add_per_turn', true ),
		"inflation_multiplier_total" => (double)get_post_meta( $gameID, '_inflation_multiplier_total', true ),
	);
	return $statsNow;
}
...
function adjustGovermentPollingBasedOnPerformance($gameID, $election) {
	$country_instance = json_decode(get_post_meta($gameID, '_country_instance', true), true);
	...
	foreach ($statsPrevTurn as $key => $oldstat) {
		...
		if ($key == 'goverment_credit_rating') {
			...
			$creditRatingNow = $statsNow['goverment_credit_rating'];
