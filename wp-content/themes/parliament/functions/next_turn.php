...
function game_next_turn_function()
{
	...
	// Count inflation
	$inflationToAdd = (float)get_post_meta($gameID, '_inflation_add_per_turn', true);
	$inflationMultiplierTotal = (float)get_post_meta($gameID, '_inflation_multiplier_total', true) + $inflationToAdd;
	...
	if ($eventTriggersRand == 1) {
		...
		while ($loop->have_posts()) : $loop->the_post();
			...
			if ($population_change == 1) {
				...
				$population_change = (float)$population_change;
			}
		...
		$country_instance['gdp'] = round((int)$country_instance['gdp'] * (float)$theEvent['gdp_multiplier']);
		$country_instance['population'] = round((int)$country_instance['population'] * (float)$theEvent['population_multiplier']);
